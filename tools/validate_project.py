...
recording_lifecycle = text("feature/tools/src/main/java/com/apextuner/feature/tools/recording/RecordingLifecycleGate.kt")
recording_lifecycle_test = text("feature/tools/src/test/java/com/apextuner/feature/tools/recording/RecordingLifecycleGateTest.kt")
projection_pos = recording_service.find("projectionManager.getMediaProjection(resultCode, resultData)")
callback_registration_pos = recording_service.find("projection.registerCallback(projectionCallback, mainHandler)")
drain_assignment_pos = recording_service.find("drainThread = thread")
drain_start_pos = recording_service.find("thread.start()")
running_publish_pos = recording_service.find("onStarted(this@RecordingSession)")
...
check(projection_pos >= 0 and callback_registration_pos > projection_pos, "MediaProjection token must be resolved before lifecycle callback registration")
check("HandlerThread" not in recording_service, "Screen recorder MediaProjection callback must stay serialized on the Service main looper")
check("projection.registerCallback(projectionCallback, mainHandler)" in recording_service, "MediaProjection callback must use the Service main looper")
check("private val lifecycle = RecordingLifecycleGate()" in recording_service and recording_service.count("lifecycle.locked {") >= 3, "Screen recorder startup/stop/finalization must share one lifecycle monitor")
check(drain_assignment_pos >= 0 and drain_start_pos > drain_assignment_pos, "Screen recorder must publish the drain-thread reference before starting the thread")
check(drain_start_pos >= 0 and running_publish_pos > drain_start_pos, "Screen recorder must publish Recording only after the drain thread has started")
check("if (session === owner)" in recording_service, "Screen recorder completion/state callbacks must be owner-checked")
check("if (finalSuccess && !muxerStopped)" in recording_service, "Screen recorder must not publish a file when MediaMuxer finalization fails")
for phase in ["New", "Starting", "Running", "Stopping", "Finished"]:
    check(phase in recording_lifecycle, f"Screen recording lifecycle gate missing phase: {phase}")
check("synchronized(monitor)" in recording_lifecycle, "Screen recording lifecycle gate must serialize structural transitions")
check("stopCannotInterleaveWithStartupTransaction" in recording_lifecycle_test, "Screen recorder must retain startup/stop concurrency regression coverage")
check("duplicateStopAndFinishAreIdempotent" in recording_lifecycle_test, "Screen recorder must retain idempotent stop/finish regression coverage")
